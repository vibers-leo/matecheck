...
**설명**: 이메일로 비밀번호 재설정 토큰을 발송합니다. 보안을 위해 이메일 존재 여부와 관계없이 동일한 응답을 반환합니다.
...
**설명**: 발급받은 리셋 토큰과 새 비밀번호를 제출하여 비밀번호를 변경합니다. 토큰은 발급 후 2시간 동안 유효합니다.

**파라미터**:

| 필드 | 타입 | 필수 | 설명 |
|------|------|------|------|
| token | string | O | 이메일로 발송된 리셋 토큰 |
| password | string | O | 새 비밀번호 (최소 6자) |
